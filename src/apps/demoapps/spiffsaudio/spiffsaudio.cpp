...
bool SpiffsAudio::create() {
    ...
    playButton = styles.stdButton(bg, "Sound!");
    register_lv_event_callback(playButton);
...
void SpiffsAudio::lv_event_callback(lv_obj_t* obj, lv_event_t event) {
    if (obj==playButton && event==LV_EVENT_SHORT_CLICKED) {
        play_button();
    }
}
